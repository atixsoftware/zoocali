...
public class UniWebView: MonoBehaviour {
    ...
    Rect NextFrameRect() {
        if (referenceRectTransform == null) {
            ...
            float x = worldCorners[1].x;
            float y = Screen.height - worldCorners[1].y;
            float width = worldCorners[3].x - worldCorners[1].x;
            float height = worldCorners[1].y - worldCorners[3].y;
    ...
    public void Load(string url) {
        UniWebViewInterface.Load(listener.Name, url);
    }
    ...
    public void LoadHTMLString(string htmlString, string baseUrl) {
        UniWebViewInterface.LoadHTMLString(listener.Name, htmlString, baseUrl);
    }
    ...
    public static void SetCookie(string url, string cookie) {
        UniWebViewInterface.SetCookie(url, cookie);
    }
    ...
    public static string GetCookie(string url, string key) {
        return UniWebViewInterface.GetCookie(url, key);
    }
    ...
    /// web view by using a browser development tool of Chrome.
    /// 
    /// This method is only for Android. On iOS, you do not need additional step. 
