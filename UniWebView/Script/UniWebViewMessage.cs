...
public struct UniWebViewMessage {
    ...
    public UniWebViewMessage(string rawMessage): this() {
        ...
        string[] schemeSplit = this.RawMessage.Split(new string[] {"://"}, System.StringSplitOptions.None);
        if (schemeSplit.Length >= 2) {
            ...
            if (split.Length > 1) {
                foreach (string pair in split[1].Split("&"[0])) {
                    ...
                    if (elems.Length > 1) {
                        Args[elems[0]] = elems[1];
                        UniWebViewLogger.Instance.Debug("Get arg, key: " + elems[0] + " value: " + Args[elems[0]]);
                    }
                }
            }
        } else {
